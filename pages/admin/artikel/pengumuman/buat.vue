...
async function publishArticle() {
  ...
  announcement.value.flags = ArticleFlags.IS_PUBLISHED;

  const res = await $api.articles.createAnnouncement(announcement.value.image_url, announcement.value.title, announcement.value.summary, announcement.value.content!, announcement.value.flags, token.value!);
  if (!res.success) {
    // @ts-ignore
    $swal.fire({
      title: "Operasi gagal",
...
async function draftArticle() {
  ...
  // Update article content
  announcement.value.content = editorContentDelta.value;
  announcement.value.flags = ArticleFlags.IS_DRAFT;
...
<template>
  <Header title="Buat Pengumuman"></Header>
  ...
  <div class="mt-12 mb-12 container mx-auto max-w-[770px] px-8 lg:px-0">
    ...
    <img id="preview-img" :src="announcement.image_url" alt="Article header image" class="mb-4 rounded-md aspect-video w-full object-contain" style="background-color: #d0d1d3;">
