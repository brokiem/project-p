...
const {message: articleCountResponse} = await $api.articles.getAnnouncementCount(token.value!);
...
const searchArticle = async () => {
  if (searchValue.value) {
    await navigateTo(`/admin/artikel/pengumuman/cari?q=${searchValue.value}`);
  }
};
</script>
      ...
      <div class="py-8 px-4 mx-auto md:max-w-[1250px] lg:py-16 lg:px-6">
        ...
        <div class="max-w-screen-xl mb-8 lg:mb-10">
          <div class="flex flex-row gap-x-4">
            ...
            <!-- Create icon -->
            <NuxtLink to="/admin/artikel/pengumuman/buat" class="flex items-center justify-center bg-blue-600 hover:bg-blue-700 w-10 h-10 text-white rounded-full text-sm">
        ...
        <AdminArticlesContainer
            :announcements-starting-index="startingArticleIndex"
            :display-announcements="true"
            :number-of-announcements-to-display="articlesPerPage"
            class="mb-8"
        />
        ...
        <Pagination
            ...
            pagination-base-url="/admin/artikel/pengumuman/page"
        />
      </div>
    </div>
  </div>
