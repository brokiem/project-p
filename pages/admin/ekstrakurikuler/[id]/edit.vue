...
const extracurricularId = parseInt(route.params.id as string);
const token = useCookie("token");

// Check if extracurricularId is a number
if (isNaN(extracurricularId)) {
  // Return nuxt 404 page
  throw error404();
}

// @ts-ignore
const extracurricular: Ref<{ id: number; title: string; description: string; extracurricular_mentors: { id: number; users: User; }[]; }> = ref([]);

async function loadExtracurriculars() {
  const {$api} = useNuxtApp();
  const {message} = await $api.extracurriculars.getById(extracurricularId);

  extracurricular.value.id = message.extracurricular.id;
  extracurricular.value.title = message.extracurricular.title;
  extracurricular.value.description = message.extracurricular.description;
  extracurricular.value.extracurricular_mentors = message.extracurricular.extracurricular_mentors;
}
...
try {
  ...
  extracurricular.value.extracurricular_mentors.forEach((mentor) => {
    selectedMentors.value.push(mentor.users);
  });
} catch (err) {
  console.error(err);
}
...
function isDataValid() {
  return extracurricular.value.title.trim() !== "" && extracurricular.value.description.trim() !== "";
}
...
async function editExtracurricular() {
  ...
  if (!isDataValid()) {
    ...
    $swal.fire({
      title: "Gagal!",
      text: "Judul dan deskripsi ekstrakurikuler tidak boleh kosong",
      icon: "error",
  ...
  const {message} = await $api.extracurriculars.edit(extracurricularId, extracurricular.value.title, extracurricular.value.description, selectedMentorsUuids, token.value!);
...
async function deleteExtracurricular() {
  const {$api, $swal} = useNuxtApp();
  ...
  $swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      const {message} = await $api.extracurriculars.delete(extracurricularId, token.value!);
...
<template>
  <Header title="Edit Ekstrakurikuler"></Header>
  ...
  <div class="mt-12 mb-12 container mx-auto max-w-[770px] px-8 lg:px-0">
    ...
    <div>
      ...
      <button data-modal-target="add-mentor-modal" data-modal-toggle="add-mentor-modal"
              class="transition duration-150 mt-2 text-gray-900 border-gray-300 bg-white hover:bg-gray-100 shadow border focus:ring-2 focus:ring-gray-600 font-medium rounded-md text-sm px-5 py-1 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" type="button">
        Pilih Guru Pembimbing
      </button>
    </div>

    <!-- Delete Ekstrakurikuler button -->
    <button
