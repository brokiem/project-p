...
if (!searchQuery) {
  await navigateTo("/artikel/berita/page/1");
}
...
const searchArticle = async () => {
  if (searchValue.value) {
    ...
    await navigateTo(`/artikel/berita/cari?q=${encodeURIComponent(searchValue.value)}`);
  }
};
</script>
        ...
        <form class="flex items-center mb-4" @submit.prevent="searchArticle">
          ...
          <div class="relative w-full">
            <input id="searchInput" v-model="searchValue"
                   ...
                   placeholder="Pencarian" required type="search">
          </div>
